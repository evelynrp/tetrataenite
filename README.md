...
- **blujust** - Easy access to common administrative tasks
- **nvidia-open** - Nvidia's open-source kernel modules (optional)
- **Bazaar** - A fast, beautiful, and modern app store
- **Google Chrome** - Browse safely and securely with custom policies and flags
- **Mullvad VPN** - Stay private on untrusted networks
- **VS Code** - A fast, light, modular IDE
...
```bash
rpm-ostree rebase ostree-unverified-registry:ghcr.io/evelynrp/tetrataenite-gnome-nvidia
systemctl reboot

rpm-ostree rebase ostree-image-signed:docker://ghcr.io/evelynrp/tetrataenite-gnome-nvidia
systemctl reboot
```

Replace `tetrataenite-gnome-nvidia` with your chosen image from this list:
- tetrataenite-cosmic
...
BlueBuild's base images handle most of the heavy lifting. The decisions below are what tetrataenite adds on top.
...
Mullvad VPN and VS Code are baked in because Mullvad VPN has no flatpak and the VS Code flatpak has known issues. I did consider using [uBlue's homebrew tap](https://github.com/ublue-os/homebrew-tap) to enable users to install VS Code on their own, but brew's limitations in multi-user environments ultimately pushed me to bake it in.

[Hack Nerd](https://www.nerdfonts.com/font-downloads) is included because I like it, and Microsoft fonts are included because my coworkers all use Microsoft Office.